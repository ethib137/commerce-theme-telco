<div class="telco-user-nav" tabindex="0">
	<#if is_signed_in>
		<#assign
		userImageCssClass = ""
		/>
		...
		<div class="telco-user-nav__avatar ${userImageCssClass}">
			<@liferay_ui["user-portrait"] user=user />

			<div class="telco-user-nav__name">${user_name}</div>
		</div>

		<div class="telco-user-nav__menu">
			...
			<a class="main-link main-link--sub" href="${notification_url}">
				<div class="main-link__label">
					${notifications_text}:
					<div class="telco-notification-badge">${notification_count}</div>
				</div>
			</a>
	...
	<#else>
		<div class="telco-user-nav__sign-in">
